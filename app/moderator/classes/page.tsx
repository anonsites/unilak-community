...
export default function ClassesManagementPage() {
  ...
  const [sortBy, setSortBy] = useState<'course' | 'department' | 'updated'>('updated');
  ...
  }, [refreshKey, sortBy, filterDept, filterProgram, searchTerm]);
      ...
      <div className="bg-gray-900 text-white p-4 md:p-6 rounded-xl shadow-lg mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-2">
            Total Classes: <span className="font-semibold text-white">{filteredClasses.length}</span>
          </p>
        </div>
      ...
      <div className="bg-gray-900 text-white p-4 md:p-6 rounded-xl shadow-lg mb-6 space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div>
            ...
            <select
              value={sortBy}
              onChange={(e) => setSortBy(e.target.value as 'course' | 'department' | 'updated')}
              className="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-white focus:outline-none focus:border-blue-500"
            >
        ...
        {/* Clear Filters */}
        {(searchTerm || filterDept || filterProgram) && (
          <button
            onClick={() => {
              setSearchTerm('');
              setFilterDept('');
              ...
              <tbody>
                {filteredClasses.map((cls, index) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-sm font-medium text-white truncate">{cls.course_name}</td>
